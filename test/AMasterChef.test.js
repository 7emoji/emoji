...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should set correct state variables', async () => {
        this.chef = await MasterChef.new(this.emoji.address, dev, '1000', '0', '1000', { from: alice });
        await this.emoji.transferOwnership(this.chef.address, { from: alice });
        const emoji = await this.chef.emoji();
        const devaddr = await this.chef.devaddr();
        const owner = await this.emoji.owner();
        assert.equal(emoji.valueOf(), this.emoji.address);
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await MasterChef.new(this.emoji.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should give out EMOJIs only after farming time', async () => {
            ...
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '0');
            ...
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '1000');
            await time.advanceBlockTo('104');
            await this.chef.deposit(0, '0', { from: bob }); // block 105
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '5000');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '500');
            assert.equal((await this.emoji.totalSupply()).valueOf(), '5500');
        });

        it('should not distribute EMOJIs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.chef = await MasterChef.new(this.emoji.address, dev, '100', '200', '1000', { from: alice });
            await this.emoji.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            await time.advanceBlockTo('199');
            assert.equal((await this.emoji.totalSupply()).valueOf(), '0');
            ...
            assert.equal((await this.emoji.totalSupply()).valueOf(), '0');
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '0');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '990');
            await time.advanceBlockTo('219');
            await this.chef.withdraw(0, '10', { from: bob }); // block 220
            assert.equal((await this.emoji.totalSupply()).valueOf(), '11000');
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '10000');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '1000');
            assert.equal((await this.lp.balanceOf(bob)).valueOf(), '1000');
        });

        it('should distribute EMOJIs properly for each staker', async () => {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChef.new(this.emoji.address, dev, '100', '300', '1000', { from: alice });
            await this.emoji.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            await this.chef.deposit(0, '10', { from: alice });
            assert.equal((await this.emoji.totalSupply()).valueOf(), '11000');
            assert.equal((await this.emoji.balanceOf(alice)).valueOf(), '5666');
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '0');
            assert.equal((await this.emoji.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.emoji.balanceOf(this.chef.address)).valueOf(), '4334');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '1000');
            // Bob withdraws 5 LPs at block 330. At this point:
            //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
            await time.advanceBlockTo('329')
            await this.chef.withdraw(0, '5', { from: bob });
            assert.equal((await this.emoji.totalSupply()).valueOf(), '22000');
            assert.equal((await this.emoji.balanceOf(alice)).valueOf(), '5666');
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '6190');
            assert.equal((await this.emoji.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.emoji.balanceOf(this.chef.address)).valueOf(), '8144');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '2000');
            ...
            assert.equal((await this.emoji.totalSupply()).valueOf(), '55000');
            assert.equal((await this.emoji.balanceOf(dev)).valueOf(), '5000');
            // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
            assert.equal((await this.emoji.balanceOf(alice)).valueOf(), '11600');
            // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
            assert.equal((await this.emoji.balanceOf(bob)).valueOf(), '11831');
            // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
            assert.equal((await this.emoji.balanceOf(carol)).valueOf(), '26568');
        ...
        it('should give proper EMOJIs allocation to each pool', async () => {
            ...
            this.chef = await MasterChef.new(this.emoji.address, dev, '100', '400', '1000', { from: alice });
            await this.emoji.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        it('should stop giving bonus EMOJIs after the bonus period ends', async () => {
            ...
            this.chef = await MasterChef.new(this.emoji.address, dev, '100', '500', '600', { from: alice });
            await this.emoji.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
            ...
            assert.equal((await this.emoji.balanceOf(alice)).valueOf(), '10600');
        });
    });
});
