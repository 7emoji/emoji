...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to EmojiSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // EmojiSwap must mint EXACTLY the same amount of EmojiSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of EMOJIs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. EMOJIs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that EMOJIs distribution occurs.
        uint256 accEmojiPerShare; // Accumulated EMOJIs per share, times 1e12. See below.
    }
    ...
    // Update the given pool's EMOJI allocation point. Can only be called by the owner.
    ...
    // View function to see pending EMOJIs on frontend.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accEmojiPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accEmojiPerShare).div(1e12);
